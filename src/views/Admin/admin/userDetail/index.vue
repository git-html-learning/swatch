  ...
  <div >
    ...
    >
      ...
      <el-form-item :label="user.userAddress">
        <el-select v-model="userData.userAddress.province" placeholder="省份">
          <el-option
      ...
      <el-form-item v-show="title == 'edit'">
        <el-row>
...
<script>
import { OtherUserDetail, AdminEditUser } from "@/api/admin";
export default {
  data() {
  ...
  methods: {
    async requireUserData() {
      let user = await OtherUserDetail(this.otherUserData);
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        console.log(valid);
        if (valid) {
          var res = await AdminEditUser(this.userData);
          if (res.code == 200) {
            this.$message({
              showClose: true,
              message: "用户信息更新成功",
