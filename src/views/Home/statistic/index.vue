<template>
  <div>
    <el-row :gutter="40" class="panel-group">
      <router-link to="/products/index">
        <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
          <div class="card-panel">
            <div class="card-panel-icon-wrapper icon-people">
              <svg-icon icon-class="产品" class-name="card-panel-icon" />
            </div>
            <div class="card-panel-description">
              <div class="card-panel-text">产品数量</div>
              <!-- 开始值，结束值，持续时间：毫秒 -->
              <count-to
                :start-val="0"
                :end-val="productnums"
                :duration="2600"
                class="card-panel-num"
              />
            </div>
          </div>
        </el-col>
      </router-link>
      <router-link to="/devices/index">
        <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
          <div class="card-panel">
            <div class="card-panel-icon-wrapper icon-message">
              <svg-icon icon-class="dashboard" class-name="card-panel-icon" />
            </div>
            <div class="card-panel-description">
              <div class="card-panel-text">设备数量</div>
              <count-to
                :start-val="0"
                :end-val="devicenums"
                :duration="3000"
                class="card-panel-num"
              />
            </div>
          </div>
        </el-col>
      </router-link>
      <router-link to="/devices/index">
        <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
          <div class="card-panel">
            <div class="card-panel-icon-wrapper icon-money">
              <svg-icon icon-class="在线" class-name="card-panel-icon" />
            </div>
            <div class="card-panel-description">
              <div class="card-panel-text">在线设备</div>
              <count-to
                :start-val="0"
                :end-val="statusnums"
                :duration="3200"
                class="card-panel-num"
              />
            </div>
          </div>
        </el-col>
      </router-link>
      <router-link to="/devices/index">
        <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
          <div class="card-panel">
            <div class="card-panel-icon-wrapper icon-shopping">
              <svg-icon icon-class="离线" class-name="card-panel-icon" />
            </div> 
            <div class="card-panel-description">
              <div class="card-panel-text">离线设备</div>
              <count-to
                :start-val="0"
                :end-val="devicenums-statusnums"
                :duration="3600"
                class="card-panel-num"
              />
            </div>
          </div>
        </el-col>
      </router-link>
    </el-row>
  </div>
</template>




<script>
import CountTo from "vue-count-to";
import {
  ProductNum,
  ProductOne,
  deviceStatus,
  deviceStatusNums,
} from "@/api/index";
export default {
  components: {
    CountTo,
  },
  created() {
    this.productNum();
  },
  computed: {
    productnums() {
      return this.$store.state.other.productNums.length;
    },
    devicenums() {
      return this.$store.state.other.deviceNums.length;
    },
    statusnums() {
      return this.$store.state.other.statusNums.length;
    },
  },

  
  // 不懂
  methods: {
    handleSetLineChartData(type) {
      this.$emit("handleSetLineChartData", type);
    },
    productNum() {
      this.$store.dispatch("productNum");
    },
  },
};
</script> 

<style lang="scss" scoped>
.panel-group {
  margin-top: 18px;
  .card-panel-col {
    margin-bottom: 32px;
  }

  .card-panel {
    height: 108px;
    cursor: pointer;
    font-size: 12px;
    position: relative;
    overflow: hidden;
    color: #666;
    background: #fff;
    box-shadow: 4px 4px 40px rgba(0, 0, 0, 0.05);
    border-color: rgba(0, 0, 0, 0.05);

    &:hover {
      .card-panel-icon-wrapper {
        color: #fff;
      }

      .icon-people {
        background: #40c9c6;
      }

      .icon-message {
        background: #36a3f7;
      }

      .icon-money {
        background: #84d944;
      }

      .icon-shopping {
        background: #f4516c;
      }
    }

    .icon-people {
      color: #40c9c6;
    }

    .icon-message {
      color: #36a3f7;
    }

    .icon-money {
      color: #84d944;
    }

    .icon-shopping {
      color: #f4516c;
    }

    .card-panel-icon-wrapper {
      float: left;
      margin: 14px 0 0 14px;
      padding: 16px;
      transition: all 0.38s ease-out;
      border-radius: 6px;
    }

    .card-panel-icon {
      float: left;
      font-size: 48px;
    }

    .card-panel-description {
      float: right;
      font-weight: bold;
      margin: 26px;
      margin-left: 0px;

      .card-panel-text {
        line-height: 18px;
        color: rgba(0, 0, 0, 0.45);
        font-size: 16px;
        margin-bottom: 12px;
      }

      .card-panel-num {
        font-size: 20px;
      }
    }
  }
}

@media (max-width: 550px) {
  .card-panel-description {
    display: none;
  }

  .card-panel-icon-wrapper {
    float: none !important;
    width: 100%;
    height: 100%;
    margin: 0 !important;

    .svg-icon {
      display: block;
      margin: 14px auto !important;
      float: none !important;
    }
  }
}
</style>
